import { Text, View, TextInput, Button, Alert, StyleSheet, Pressable, ScrollView,Image, Modal, TouchableOpacity} from "react-native";
...
const Comentarios = ({index, indice, usuario, mensaje, respuestas, arrayUpdate, idDoc, modoNocturno, tamañoLetra, admin}) =>{
    ...
    function realizarRespuesta(){
        ...
        setRespuesta("");
        Alert.alert("¡Éxito!","Su comentario se ha realizado correctamente.");
    }
    ...
    return(
        <View key={index} style={[{backgroundColor:'#fff', padding:30, marginBottom:1, flex:1}, modoNocturno ? null : {backgroundColor:'#323639', borderWidth:1}]}>
            <View>
                ...
                <Pressable
                    ...
                    <Ionicons name="send" size={20} color= {modoNocturno ? "black" : "white"} style={{position:'absolute', bottom:-10, right:0, marginTop:15}} /> 
                </Pressable>
            </View>
                    ...
                    <View style={{height: '30%', marginTop: 'auto', backgroundColor:'#fff',}}>
                        ...
                        <View style={{backgroundColor:'#fff', borderRadius:20, marginTop:5, marginLeft:10, marginRight:10}}>
                            <TextInput 
                                ...
                                style={[{padding:20, marginLeft:1, height:100, marginTop:2, borderWidth:1, borderRadius:20},  tamañoLetra ? {fontSize:18} : {fontSize:20}]}
                                editable={true}
                                multiline={true}
                            ...
                            <Pressable
                                onPress = {() => { 
                                    if(respuesta.length==0){
                                        Alert.alert("¡ Cuidado !","No puedes enviar una respuesta vacía.");
                                    }
                                    else{
                                        realizarRespuesta();
                                    }
...
export default function ForoForm({ route, navigation: { goBack } }) {
    ...
    return (
      <View style={{flex:1, backgroundColor:'#edeff1'}}>
        ...
        <ScrollView>
            <View style={[{backgroundColor:'#fff', padding:30, marginBottom:1}, modoNocturno ? null : {backgroundColor:'#323639', borderWidth:1}]}>
                <View style={{flexDirection:'row', alignItems:'center'}}>
                    <Image 
            ...
            {dejarComentario && <View style={{marginBottom:1, padding:30}}>
                ...
                <View style={{backgroundColor:'#fff', borderRadius:20}}>
                    <TextInput 
                        ...
                        style={[{padding:2, marginLeft:10, height:100, flex:1, }, tamañoNormal ? null : {fontSize:18}]}
                        editable={true}
                        multiline={true}
            ...
            {datosBD["Comentarios"] && 
             datosBD["Comentarios"].map((comentario, index)=>{
                return <Comentarios key={index} indice={index} usuario={comentario["Id_Usuario"]} mensaje={comentario["Mensaje"]} respuestas={comentario["Respuestas"]} arrayUpdate={datosBD["Comentarios"]} idDoc={route.params} modoNocturno={modoNocturno} tamañoLetra={tamañoNormal} admin={true} />
             })
            }
        </ScrollView>
      </View>
