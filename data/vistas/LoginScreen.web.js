import { StyleSheet, Text, View, Alert, Image, Dimensions, TouchableOpacity, ScrollView, TextInput, Keyboard, ImageBackground, Platform } from 'react-native';
...
export default function LoginScreen({navigation}) {
  ...
  const [alertaMensaje, setAlertaMensaje] = useState(false);
  ...
  function iniciarSesionFuncion(){
    ...
    signInWithEmailAndPassword(auth, email, password).then((userCredential) => {
        if(!userCredential.user.emailVerified){
            alert('Verifique el email para poder entrar. Consulte la bandeja de Spam.');
        }
    }).catch((error) => {
        alert('Error. El usuario y la contraseña no coinciden.');
    // ..
    });
  }
  ...
  async function registrarseFuncion(){
    try{
        ...
        if(nombreUsuario.length<3){
            alert('Error. Introduzca un nombre de usuario con al menos 3 caracteres');
            return false;
        }
        if(reg.test(emailRegistro) === false) {
            ...
            alert('Error. Introduzca una contraseña con al menos 6 caracteres');
            return false;
        }
        ...
        await createUserWithEmailAndPassword(auth, emailRegistro, passwordRegistro).then((userCredential) => {
            ...
            if (error.code === 'auth/email-already-in-use') {
                alert('Error. El email introducido ya está registrado');
                return;
            }
            else if (error.code == 'auth/weak-password') {
                alert('Error. Introduzca una contraseña más robusta');
            }  
        // ..
        });
        ...
        if(auth){
            ...
            setAlertaExito(true);
            setAlertaMensaje('Se ha enviado un correo de verificación. Por favor, verifica tu correo antes de iniciar sesión. Consulte en la carpeta de Spam.');
        }

    } catch(err){
        alert("Ha habido un error al realizar el registro");
        console.log(err);
    }
  }
  ...
  return (
    <View style={styles.container}>
      ...
      {iniciarSesion &&
      <View style={[{borderRadius:20, marginTop:30}, visualizarMovil ? null : {paddingLeft:'30%', paddingRight:'30%'}]}>
            <ScrollView>
      ...
      {!iniciarSesion &&  
        <ScrollView>
            ...
            <View style={[{marginTop:20, marginLeft:20, marginRight:20},  visualizarMovil ? null : {paddingLeft:'30%', paddingRight:'30%'}]}>
